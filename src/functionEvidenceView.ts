...
export function showFunctionEvidencePanel(
  ...
  const missing = (['cpu', 'allocation', 'live-memory'] as EvidenceKind[])
...
<body>
  ...
  <div class="intro">
    这里汇总所有已采集证据中与当前 Go 函数匹配的数据。Self 表示函数自身，With callees 包含下游调用。
  ...
  <div class="evidence">
    ${report.items.length === 0
      ...
      : report.items.map(evidenceHtml).join('')}
  </div>
  ...
  panel.webview.onDidReceiveMessage((message) => {
    if (message?.command === 'open-profile' && typeof message.sessionId === 'string') {
      ...
      && (message.kind === 'cpu' || message.kind === 'allocation' || message.kind === 'live-memory')
    ) {
...
function kindLabel(kind: EvidenceKind): string {
  ...
  if (kind === 'live-memory') return 'Live Heap';
