...
.notice{padding:10px 12px;background:var(--vscode-textBlockQuote-background);border-left:3px solid var(--vscode-charts-blue)}
</style></head><body>
...
${metricNames.length === 0
    ...
    : `<div class="notice">Raw metric differences only. GoTune does not infer whether higher or lower is better without an explicit metric direction and acceptance rule.</div><table><thead><tr><th>Metric</th><th>Baseline</th><th>Current</th><th>Delta</th></tr></thead><tbody>
...
<td>${percent === undefined ? '—' : `${percent > 0 ? '+' : ''}${percent.toFixed(1)}%`}</td></tr>`;
}
