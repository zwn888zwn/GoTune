...
<head>
  ...
  <style nonce="${nonce}">
    *{box-sizing:border-box;scrollbar-width:thin;scrollbar-color:var(--vscode-scrollbarSlider-background) transparent}
    ...
    .chart{margin-bottom:12px}.head{display:flex;justify-content:space-between;align-items:center;margin-bottom:4px}.name{font-weight:600}.value{color:var(--vscode-descriptionForeground);font-variant-numeric:tabular-nums}
    ...
    .axis{display:flex;justify-content:space-between;min-height:16px;padding-top:3px;color:var(--vscode-descriptionForeground);font-size:10px;font-variant-numeric:tabular-nums}.recording{margin-top:-4px;color:var(--vscode-errorForeground);font-variant-numeric:tabular-nums}
  </style>
</head>
<body>
  <div class="empty" id="empty">启动 Go main 或连接 pprof 服务后，这里显示最近 5 分钟的 CPU 与 Heap 趋势。</div>
  <div class="charts" id="charts" hidden>
    <section class="chart">
      <div class="head"><span class="name">CPU</span><span class="value" id="cpuValue">—</span></div>
      <svg viewBox="0 0 300 90" preserveAspectRatio="none"><path class="grid" d="M0 30H300M0 60H300"></path><polygon id="cpuArea" class="cpu-area"></polygon><polyline id="cpuLine" class="cpu-line"></polyline></svg>
    ...
    <section class="chart">
      ...
      <svg viewBox="0 0 300 90" preserveAspectRatio="none"><path class="grid" d="M0 30H300M0 60H300"></path><polygon id="heapArea" class="heap-area"></polygon><polyline id="heapLine" class="heap-line"></polyline></svg>
  ...
  <script nonce="${nonce}">
    ...
    const points=values=>{if(!values.length)return '';const max=Math.max(...values,1),min=Math.min(...values,0),span=Math.max(1,max-min);return values.map((value,index)=>{const x=values.length===1?0:index/(values.length-1)*300;const y=86-(value-min)/span*80;return x.toFixed(1)+','+y.toFixed(1)}).join(' ')};
    ...
    const render=()=>{const cpu=samples.map(item=>item.cpuPercent??0),heap=samples.map(item=>item.heapAlloc),cpuPoints=points(cpu),heapPoints=points(heap);byId('cpuLine').setAttribute('points',cpuPoints);byId('cpuArea').setAttribute('points',cpu.length?'0,90 '+cpuPoints+' 300,90':'');byId('heapLine').setAttribute('points',heapPoints);byId('heapArea').setAttribute('points',heap.length?'0,90 '+heapPoints+' 300,90':'');byId('cpuValue').textContent=cpu.length?cpu.at(-1).toFixed(1)+'%':'—';byId('heapValue').textContent=heap.length?bytes(heap.at(-1)):'—';byId('recording').textContent=recordingStartedAt?'● CPU 录制中 '+elapsed(Date.now()-recordingStartedAt):'';renderAxis('cpuAxis');renderAxis('heapAxis');byId('empty').hidden=model.targetActive;byId('charts').hidden=!model.targetActive};
